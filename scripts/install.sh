...
# Suggest package manager alternatives where available
...
download_release() {
  ...
  log_debug "Downloading from GitHub Releases: $github_url"
...
do_binary_install() {
  ...
  if [ "$curl_installed" -eq 0 ] || [ "$wget_installed" -eq 0 ]; then
    ...
    if [ "$curl_installed" -eq 0 ]; then
      ...
      status_code=$(download_release "$filename" "Binary")
...
for arg; do
  ...
  case "$arg" in
    ...
    --install-path)
      ...
      ;;
    --no-repo)
      # Accepted for backwards compatibility; direct binary install is now the only path.
      ;;
  esac
done
