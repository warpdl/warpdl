...
must_haves:
  truths:
    ...
    - "StripURLCredentials (exported) removes userinfo from URL: ftp://user:pass@host/path -> ftp://host/path (security critical)"
  ...
  artifacts:
    - path: "pkg/warplib/protocol_ftp.go"
      provides: "ftpProtocolDownloader struct, newFTPProtocolDownloader factory, Probe/Download methods, connect helper, classifyFTPError, StripURLCredentials (exported), ftpProgressWriter"
...
<feature>
  ...
  <implementation>
    ...
    - `StripURLCredentials(rawURL string) (string, error)` — exported; removes userinfo from URL. Must be exported because internal/api calls it cross-package in Plan 03-03.
