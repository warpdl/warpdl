...
must_haves:
  truths:
    ...
    - "StripURLCredentials (exported) removes userinfo from URL: ftp://user:pass@host/path -> ftp://host/path (security critical). Returns string only (no error) — the URL is already validated at factory time so url.Parse cannot fail again"
  ...
  artifacts:
    - path: "pkg/warplib/protocol_ftp.go"
      provides: "ftpProtocolDownloader struct, newFTPProtocolDownloader factory, Probe/Download methods, connect helper, classifyFTPError, StripURLCredentials (exported, returns string only — no error), ftpProgressWriter"
...
<feature>
  ...
  <implementation>
    ...
    func TestStripURLCredentials(t *testing.T) { /* tests 8, 9 — calls exported warplib.StripURLCredentials */ }
    ...
    - `StripURLCredentials(rawURL string) string` — exported; removes userinfo from URL. Returns string only (no error) because the URL was already validated by the factory. If url.Parse somehow fails, returns rawURL unchanged. Must be exported because internal/api calls it cross-package in Plan 03-03.
...
# NOTE: TestStripURLCredentials tests the exported StripURLCredentials function
