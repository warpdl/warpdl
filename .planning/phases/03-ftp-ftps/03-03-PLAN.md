...
must_haves:
  truths:
    ...
    - "Daemon runner initializes SchemeRouter and sets it on Manager via SetSchemeRouter (SetSchemeRouter is implemented in Plan 03-02)"
  ...
  artifacts:
    ...
    - path: "internal/daemon/runner.go"
      provides: "SchemeRouter initialization and Manager.SetSchemeRouter call"
  key_links:
    ...
    - from: "internal/daemon/runner.go"
      to: "pkg/warplib/protocol_router.go:NewSchemeRouter"
      via: "Daemon creates SchemeRouter and sets it on Manager"
...
@internal/api/download.go
@internal/api/resume.go
@internal/daemon/runner.go   (if exists — daemon initialization)
...
<feature>
  ...
  <files>internal/api/download.go, internal/api/download_ftp_test.go, internal/api/resume.go, internal/daemon/runner.go, pkg/warplib/protocol_ftp_test.go</files>
  <behavior>
    ...
    14. Daemon runner creates SchemeRouter with NewSchemeRouter(client) and calls manager.SetSchemeRouter(router)
  ...
  <implementation>
    ...
    ```go
    func NewApi(log *logger.Logger, mgr *warplib.Manager, engine *extl.Engine, client *http.Client, router *warplib.SchemeRouter) *Api {
        return &Api{
            log:          log,
            manager:      mgr,
            elEngine:     engine,
    ...
    PHASE D: Update daemon runner to initialize SchemeRouter

    In internal/daemon/runner.go (or wherever the daemon is started):
    ```go
    ...
    // Pass router to Api constructor
    api := api.NewApi(log, manager, engine, client, router)
    ```
    ...
    - Ensure stripURLCredentials is exported from warplib (or use pd.cleanURL via a getter)
    - Run full test suite
...
- Daemon runner initializes SchemeRouter and wires it to Manager and Api
