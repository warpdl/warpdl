...
must_haves:
  truths:
    ...
    - "Resume() derives start offset from WarpStat(savePath).Size(), not from parts map key (FTP parts always start at offset 0, so file size on disk is the source of truth for resume position)"
    ...
    - "Manager has a schemeRouter *SchemeRouter field and SetSchemeRouter(r *SchemeRouter) method — used by daemon core (Plan 03-03) and by ResumeDownload internally to create FTP/FTPS downloaders for resume dispatch"
