...
:::note
Automatic native-host installation only works when `OfficialChromeExtensionID` and `OfficialFirefoxExtensionID` are set in `internal/nativehost/defaults.go` (currently empty strings).
:::
