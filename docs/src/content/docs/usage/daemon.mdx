...
| `--speed-schedule` | - | Throttle window `HH:MM-HH:MM:SPEED` (e.g. `09:00-17:00:512KB`) |
| `--rpc-listen-all` | `false` | Bind the JSON-RPC endpoint to all interfaces instead of localhost only |

Throttle example (full speed overnight, 512 KB/s during work hours):

```bash
# Via flag
warpdl daemon --speed-schedule 09:00-17:00:512KB

# Via environment variable (used when the flag is not set)
export WARPDL_SPEED_SCHEDULE=09:00-17:00:512KB
warpdl daemon
```
...
The daemon always serves its JSON-RPC API on `/jsonrpc` (HTTP) and `/jsonrpc/ws` (WebSocket). It binds to `127.0.0.1` by default and is **unauthenticated** — there is no token or secret to configure. Only bind it to all interfaces when something else provides the access control (reverse proxy, SSH tunnel, firewall):

```bash
warpdl daemon --rpc-listen-all
```
...
export WARPDL_RPC_LISTEN_ALL=1
warpdl daemon
```
