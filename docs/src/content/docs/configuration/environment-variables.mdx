...
| `WARPDL_DAEMON_TIMEOUT` | Daemon connection timeout in seconds | `30` |
...
| `WARP_MAX_PARTS` | Maximum file segments. CLI default: 200, warplib allows unlimited (0) when not set | `200` |
| `WARP_MAX_CONN` | Maximum parallel connections. CLI default: 24, warplib fallback: 1 | `24` |
| `WARP_FORCE_SEGMENTS` | Force file segmentation (boolean flag, defaults to true) | `true` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `WARPDL_MAX_CONCURRENT` | Maximum concurrent downloads | `3` |
| `WARPDL_RPC_SECRET` | RPC authentication secret | none |
| `WARPDL_RPC_LISTEN_ALL` | Listen on all interfaces (set to "1") | `false` |

- `WARPDL_PIPE_NAME` is used for named pipe IPC by default
