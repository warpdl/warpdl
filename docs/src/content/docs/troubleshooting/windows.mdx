...
WarpDL uses the named pipe `\.\pipe\warpdl` for CLI-to-daemon communication on Windows. If WarpDL can't connect to the daemon:
...
[System.IO.Directory]::GetFiles("\\.\pipe\") | Where-Object { $_ -match "warpdl" }

# List all named pipes (requires admin)
...
# - C:\Program Files\warpdl\warpdl.exe (manual install)
```
...
- **Configuration directory**: `%LOCALAPPDATA%\warpdl\` (typically `C:\Users\<username>\AppData\Local\warpdl\`)
...
- **Official releases**: Signed builds from the [Releases page](https://github.com/warpdl/warpdl/releases) are less likely to trigger warnings, but may still appear until reputation is established
...
- **Use official releases**: Download from [GitHub Releases](https://github.com/warpdl/warpdl/releases)
...
```powershell
# Check event log (requires PowerShell 3.0+)
Get-WinEvent -FilterHashtable @{LogName='Application'; ProviderName='WarpDL'} -MaxEvents 10

# Alternative: Use Format-List for detailed output
Get-WinEvent -FilterHashtable @{LogName='Application'; ProviderName='WarpDL'} -MaxEvents 5 | Format-List

# Export events to file for debugging
Get-WinEvent -FilterHashtable @{LogName='Application'; ProviderName='WarpDL'} | Export-Csv -Path "warpdl-events.csv" -NoTypeInformation
```
