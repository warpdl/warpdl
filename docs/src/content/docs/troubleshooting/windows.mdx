...
4. Browse to `warpdl.exe` location (use `where warpdl` in Command Prompt to find it) → Next
...
- **Executable path**: Full path to `warpdl.exe` (e.g., `C:\Program Files\warpdl\warpdl.exe` or where you installed it)
...
5. Click **Add an exclusion** → Select type:
   - **Folder**: Add `%LOCALAPPDATA%\warpdl` (expand the path to use the full path like `C:\Users\<username>\AppData\Local\warpdl`)
   - **File**: Add path to `warpdl.exe`
...
Unblock-File -Path "C:\path\to\warpdl.exe"
```
