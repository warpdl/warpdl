...
```powershell
# Add inbound rule for WarpDL on private networks
...
- **Application data**: `%LOCALAPPDATA%\warpdl\`
- **Configuration directory**: `%USERPROFILE%\.config\warpdl\`
- **Executable path**: Full path to `warpdl.exe` (e.g., `C:\Program Files\warpdl\warpdl.exe`)
...
5. Click **Add an exclusion** → Select type:
   - **Folder**: Add `%LOCALAPPDATA%\warpdl` and `%USERPROFILE%\.config\warpdl`
